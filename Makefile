...
HELM_DOCS_ARGS ?= --chart-search-root=charts --template-files=README.md.gotmpl --sort-values-order=file
...
# Chart READMEs are generated by helm-docs from each chart's README.md.gotmpl and the
# `# --` comments in its values.yaml. Charts opt in by adding a README.md.gotmpl; the
# rest are listed in .helmdocsignore so their hand-written READMEs are left alone.
...
.PHONY: generate-docs
generate-docs:
	@command -v $(HELM_DOCS) > /dev/null || { \
		echo "Failed. $(HELM_DOCS) is required, run 'make install-helm-docs' or see https://github.com/norwoodj/helm-docs#installation"; \
		exit 1; \
	}
	$(HELM_DOCS) $(HELM_DOCS_ARGS)
...
check-docs: generate-docs
	@if git diff --exit-code -- 'charts/*/README.md'; then \
		...
		echo "Failed. run 'make generate-docs' and commit the re-generated chart READMEs"; \
		exit 1; \
	fi
