...
- [helm-docs](https://github.com/norwoodj/helm-docs) (for generating the chart READMEs)
...
Regenerate with `make generate-docs` and verify with `make check-docs`. Both call `helm-docs`
directly; `make install-helm-docs` installs the pinned version from `HELM_DOCS_VERSION`.
`make check-docs` re-generates and then fails on `git diff`, so it leaves the regenerated
files in place when it fails - commit them.
The `helm-docs` job in the `Lint Charts` workflow runs the same check, so a stale `README.md`
fails CI.
