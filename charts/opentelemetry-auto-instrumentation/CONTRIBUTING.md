# Auto Instrumentation Chart Contributing Guide
...
Once the chart version is bumped, the examples must be regenerated.  You can regenerate examples by running `make generate-examples CHARTS=opentelemetry-auto-instrumentation`.

## Bumping Default Auto Instrumentation Version

1. Increase the minor version of the chart by one and set the patch version to zero.
2. Update the chart's `appVersion` to match the new manager version.  This version will be used as the image tag by default.
...
4. Run `make generate-examples CHARTS=opentelemetry-auto-instrumentation`.
