...
`presets.annotationDiscovery.logs.enabled: true`: Collects logs from all pods by-default, and allows to define additional configuration through annotations. Log collection from specific Pods/containers, can be disabled by using the proper annotation.
